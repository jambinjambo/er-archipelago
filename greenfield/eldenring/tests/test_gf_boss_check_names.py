...
def test_the_worked_examples_name_their_boss(flag, boss):
    ...
    assert len(hits) == 1, "flag %d matched %d checks, expected exactly 1" % (flag, len(hits))
    assert _descriptor(hits[0][1]) == boss, (
        "%r should be described by %r, got %r" % (hits[0][1], boss, _descriptor(hits[0][1])))
