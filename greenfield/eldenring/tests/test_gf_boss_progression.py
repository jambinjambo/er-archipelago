...
GAME = "Elden Ring"
BOSS_TAGS = {"Boss", "MajorBoss", "LegacyBoss", "FieldBoss", "Remembrance", "GreatRune"}
...
def test_mode_is_forced_strict_so_the_foreign_bar_is_installed():
    """🛑 The load-bearing one for §1. confined_surface_ids() returns None when this is 0, core then
    installs NO foreign-advancement bar, and the seed reads as boss-gated while quietly not being."""
    assert ps._mode(_Stub(boss_progression=1)) == 2
    assert ps._mode(_Stub(boss_progression=1, progression_surface_mode=0)) == 2
    # off -> untouched
    assert ps._mode(_Stub(boss_progression=0, progression_surface_mode=0)) == 0
...
class BossProgressionOffIsInert(WorldTestBase):
    ...
    def test_off_resolves_exactly_as_before(self):
        ...
        assert ps._mode(self.world) == int(self.world.options.progression_surface_mode.value)
...
class ForeignProgressionIsHeldToOurBosses(WorldTestBase):
    ...
    def _foreign(self, advancement):
        """An item-like belonging to another player. The bar is a pure predicate over
        .player/.advancement, so a stand-in is enough and does not need a second real world."""
        return type("I", (), {"player": self.world.player + 1,
